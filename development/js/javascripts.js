...
$(document).ready(function() {
    ...
    $(".errors").parent().addClass("has-warning");
    $(".errors").parent().prev().addClass("has-warning");

    $("input[type='radio']").parents("dd").addClass("btn-group");
    $("input[type='radio']").parents("dd").attr("data-toggle","buttons");
    $("input[type='radio']").parents("label").addClass("btn btn-primary");

    $(".list-ga-user #start_date-element, .list-ga-user #exp_date-element, .list-ga-admin #start_date-element, .list-ga-admin #exp_date-element, .vendor-search #c_create-element, .vendor-search #c_expire-element").addClass("col-md-3");
    $("fieldset .col-md-3").attr("class" , "col-md-6");
    $("#fieldset-start_date input, #c_create-element input").attr("value" , "Start date");
    $("#exp_date-element input, #c_expire-element input").attr("value" , "Expire date");
    $("form").append("<div class='clearfix'></div>");
    if($("label.btn input[checked]"))
    {
        $("label.btn input[checked]").parent().addClass("active srsrsrsrsrsrsrsr");
    }
    $("[data-toggle='buttons']").attr("data-toggle","my_buttons");
    if($(".list-group-item label input[checked]"))
    {
        $(".list-group-item label input[checked]").closest('.list-group-item').addClass("active odododododod");
        $("input[checked]").closest('.btn').addClass("active");
        $(".list-group-item label input[checked]").parents('.list-group .collapse').toggleClass("collapse").addClass("in moamennn");
        $(".collapse").prev('.collapseLink').addClass('collapsed');
    }
    $("[data-toggle='my_buttons'] input[type='radio'], [data-toggle='my_buttons'] input[type='checkbox']").click(function() {
        $("input[type='radio']").closest("[data-toggle='my_buttons']").find(".active").removeClass("active");
    ...
    $(".list-group-item label").prepend("<span class='glyphicon glyphicon-ok'></span>");

    $(".list-group-item:has(ul)").removeClass("btn").addClass("hasChilds");
    $(".btn-group input[type='submit'], .btn-group input[type='reset'], .btn-group button").addClass("btn");

    $(".hasChilds").removeClass("active");

    if($(".errors") == true) {
        var errorsElm = $(".errors:first");
        var errorsPos = errorsElm.position();
        $(window).scrollTop(errorsPos.top);
    }
    $(".carousel").each(function(){

      var itemsLength = $(".carousel-inner .item", this).length;
      var items = ("");
      
      for ( var i = 0; i < itemsLength; i++ ) {
        items += ("<li data-target='#" + $(this).attr("id") +"' data-slide-to='" + i + "'></li>");
      }
      var yy = ("<ol class='carousel-indicators'>" + items +"</ol>");

      $(this).prepend(yy);
      $(".carousel-indicators li:first", this).addClass("active");
    ...
    $('.carousel').carousel();
    $('<span class="glyphicon glyphicon-chevron-up"></span>').appendTo(".published-tender .jcarousel-prev");
    $('<span class="glyphicon glyphicon-chevron-down"></span>').appendTo(".published-tender .jcarousel-next");
    $(".published-tender .jcarousel-prev, .published-tender .jcarousel-next").addClass("btn btn-danger").appendTo(".published-tender .btn-group");

    $('<span class="glyphicon glyphicon-chevron-up"></span>').appendTo(".published-direct-invitations .jcarousel-prev");
    $('<span class="glyphicon glyphicon-chevron-down"></span>').appendTo(".published-direct-invitations .jcarousel-next");
    $(".published-direct-invitations .jcarousel-prev, .published-direct-invitations .jcarousel-next").addClass("btn btn-danger").appendTo(".published-direct-invitations .btn-group");

    $(".navbar .nav > li").each(function(){
      $("> a",this).css("width",$(this).width());
    });
    $(".media.list-group-item").parents(".panel-body").removeClass("panel-body");
    $('.required').each(function (a,b){
        $(b).html($(b).text()+'<span style="color:#c00">*</span>');
    });
    $(".user-welcome").tooltip('show');
    if($(".alert")) {
        $(function() {
        setTimeout(function() {
            $(".alert").fadeOut()
